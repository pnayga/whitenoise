"""
make_radiojove_guide_pdf.py
Convert RADIOJOVE_GUIDE.html to RADIOJOVE_GUIDE.pdf using fpdf2.

Run from the radiojove/ subfolder:
    py -3 make_radiojove_guide_pdf.py

Or from the repo root:
    py -3 whitenoise/radiojove/make_radiojove_guide_pdf.py
...
_HTML  = os.path.join(_HERE, 'RADIOJOVE_GUIDE.html')
_OUT   = os.path.join(_HERE, 'RADIOJOVE_GUIDE.pdf')
...
class GuidePDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 6, 'whitenoise - RadioJOVE SWNA Guide', align='L')
        self.ln(0.5)
...
def build_pdf():
    ...
    title  = title_block.content if title_block else 'RadioJOVE SWNA Guide'
    sub    = 'SWNA on Radio Burst Observations | wn.radiojove submodule'
